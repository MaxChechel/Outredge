...
| 11 | **XBOW withheld as work.** | The export's XBOW body is Replit's Vibecon copy **verbatim** — every paragraph of brief, solution and result is byte-identical. Per ruling, XBOW is out of scope as work: no case study, no work-grid entry, no name in body copy or meta descriptions. It is kept as a trust mark in the hero logo strip only. The draft and its remaining assets are dormant in `drafts/`; `/case-studies/xbow` 301s to `/work` because the old URL is indexed. | `drafts/`, `_redirects` |
